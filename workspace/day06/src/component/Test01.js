...
const initialState = 0 //초기값 잡기 

//함수
const reducer = (state, action) => {
    switch(action.type){ //action의 타입으로 나눠주기
        case 'INCREMENT' : 
            return state + 1
        case 'DECREMENT' : 
            return state - 1
...
const Test01 = () => {
    //count -> state 에게 전달되고, dispatch -> action으로 전달된다
    //reducer 함수는 state, action을 받아서 새로운 state 함수를 반환한다.
